     ...
     $.ajax(
         {
             ...
             success: function (resp) {
                 if (resp.status == 'ok') {
                     for (i = 0; i < resp.count; i++) {
                         hotelnames = resp.entries[i].hotelnames;
                         address = resp.entries[i].address;
                         contact_num = resp.entries[i].contact_num;
                         price = resp.entries[i].price;
                         $("#viewsearchh").append(viewsearchh(hotelnames, address, contact_num, price));
                     }
                 }
                 else {
                     $("viewsearchh").html("");
...
function viewsearchh(hotelnames, address, contact_num, price)
 {
     return '<tr>'+
